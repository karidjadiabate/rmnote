...
<style>
    ...
    .colorf {
        background-image: linear-gradient(45deg, #bb0e0e00 99%, #293D7A 50%), linear-gradient(135deg, #293D7A 50%, #ad161600 50%), linear-gradient(to right, #4A41C5, #4A41C5) !important;
    }
...
<body>
    ...
    <!-- accueil -->
    <!-- titre -->
    <div class="container text-center printableArea principal pt-3">
    ...
    <div class="banner">
        ...
        <img class="img-fluid" src="{{ asset('assets/img/Bannière_mod.jpg') }}" alt="User 1" srcset="">
    </div>
    <!-- la suite -->
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="col-md-12 position-relative perfo p-2 mb-2">
                            <div class="d-flex justify-content-between align-items-center  p-3 mb-4 ">
                                ...
                                <div class="d-flex filterdash col-md-8">
                                    <select class="form-select  me-2 custom-select classic colorf col-md-10"
                                        id="filiereSelect">
                                        <option selected>Filière...</option>
                                        <option value="1">Option 1</option>
                                        <option value="2">Option 2</option>
                                    </select>
                                    <select class="form-select w-auto custom-select classic colorf" id="classeSelect">
                            ...
                            <div class="chart-placeholder mb-4">
                                <div class="box">
                                    <canvas id="chart01" width="719" height="200"
                                        style="display: block;height: 200px!important;width: 719px!important;"></canvas>
                                </div>
                            </div>
                        </div>
                            ...
                            <table class="tables text-start  ">
                                <tbody>
                                    @foreach ($sujetgenererecents as $sujetgenererecent)
                                        <tr>
                                            <td data-label="" style="font-weight: bold">
                                                {{ $sujetgenererecent->matiere->nommatiere }}</td>
                                            <td data-label="Date de création">
                                                {{ \Carbon\Carbon::parse($sujetgenererecent->created_date)->format('d - m - Y') }}
                                            </td>
                                            <td data-label="">{{ $sujetgenererecent->typeSujet->libtypesujet }}</td>
                                            <td data-label="">
                                                {{ $sujetgenererecent->filiere->nomfiliere ?? $sujetgenererecent->filiere->etablissementFilieres->nomfilieretablissement }}
                                            </td>
                                            <td data-label="">{{ $sujetgenererecent->classe->nomclasse }}</td>
                                            <td data-label="">
                                                <span
                                                    style="background-color: {{ $sujetgenererecent->status === 'corrige' ? '#9FE4B6' : '#F9D465' }};
                                                       color: {{ $sujetgenererecent->status === 'corrige' ? '#024802' : '#6E5400' }};
                                                       padding: 0px 10px;
                                                       border-radius: 20px;">
    ...
    <script>
        // Options de données
        const dataOptions = {
            option1: [55, 61, 85, 40, 70, 90, 0, 0, 0, 0], // Correction apportée ici
            option2: [30, 50, 70, 90, 80, 60, 0, 0, 0, 0],
            option3: [10, 0, 30, 0, 50, 0, 0, 0, 0, 0]
        };

        const labelsOptions = {
            option1: ["IDA", "FCGE", "RIT", "CV", "SEI", "GBAT", "MATH-INFO", "PC", "PHYLO", "SVT", "LV-1", "LV-2"],
            option2: ["IDA", "FCGE", "RIT", "CV", "SEI", "GBAT", "MATH-INFO", "PC", "PHYLO", "SVT", "LV-1", "LV-2"],
            option3: ["DAS", "COMPTABILITE", "RESEAU", "COMMUNICATION", "SYSTEME", "BATIMENT", "MATHEMATIQUE",
                "PHYSIQUE", "PHYLOSOPHIE"
            ],
        };
        ...
        // Créer le graphique
        function createChart(initialData, initialLabels) {
            const ctx = document.getElementById("chart01").getContext('2d');
            chart = new Chart(ctx, {
                type: 'bar',
                data: {
                ...
                options: {
                    ...
                    scales: {
                        x: {
                            type: 'category', // Assurez-vous que l'axe x est de type 'category'
                            barPercentage: 0.1, // Réduit la largeur des barres à 50% de l'espace disponible
                            categoryPercentage: 0.1, // Ajuste l'espacement global entre les catégories
                            maxBarThickness: 30, // Définit une largeur maximale pour les barres
                        },
                        y: {
                            beginAtZero: true,
                            max: 100,
                            ticks: {
                                callback: function(value) {
                                    return value + '%'; // Afficher les valeurs en pourcentage
                                }
                            }
                        }
                    }
        ...
        function updateChartData(newData, newLabels) {
            if (chart) {
                ...
                chart.data.labels = newLabels; // Mettre à jour les labels
                chart.update();
            } else {
                console.error("Le graphique n'est pas défini");
            }
        }

        // Fonction pour gérer les changements de sélection
        function updateData() {
            const filiereValue = $('#filiereSelect').val();
            const classeValue = $('#classeSelect').val();

            let newData = dataOptions.option1; // Valeurs par défaut
            let newLabels = labelsOptions.option1; // Labels par défaut

            // Vérification des sélections pour définir les données et les labels
            if (filiereValue === "1") {
                newData = dataOptions.option1; // Option 1
                newLabels = labelsOptions.option1; // Labels pour option 1
            } else if (filiereValue === "2") {
                newData = dataOptions.option2; // Option 2
                newLabels = labelsOptions.option2; // Labels pour option 2
            }

            if (classeValue === "1") {
                newData = newData.map(value => value + 10); // Augmenter de 10 pour classe 1
                newLabels = labelsOptions.option3; // Labels pour option 2

            } else if (classeValue === "2") {
                newData = newData.map(value => value - 10); // Diminuer de 10 pour classe 2
                newLabels = labelsOptions.option3; // Labels pour option 2

            }

            // Mettre à jour le graphique avec les nouvelles données et labels
            updateChartData(newData, newLabels);
        }

        // Événements des sélections
        $('#filiereSelect, #classeSelect').on('change', updateData);

        // Déclenche le graphique lorsque le document est prêt
        $(document).ready(function() {
            createChart(dataOptions.option1, labelsOptions
                .option1); // Commencer avec les données et labels de l'option 1
        });
    </script>
    <script>
